// 2019-06-13 http://wenshu.court.gov.cn/Assets/js/20180919/Lawyee.CPWSW.List.js 
(function ($) {
    ...
    GetList.prototype = {
        ...
        BuildHead: function () {
            ...
            for (var i = 0; i < orderlist.length; i++) {
                ...
                $divSort.click(function () {
                    ...
                    s.BuildList(s, 1, 5, 1, Math.random(), valiguid);
                    //                    $.ajax({
                    ...
                    if (datalist != undefined && datalist != null) {
                        if (datalist.length > 1) {
                            ...
                            for (var i = 1; i < datalist.length; i++) {
                                ...
                                //不走后台xml解析处理，直接这里判断就行wj2019年1月17日14:31:04
                                var tmpCaseType = "";
                                switch (caseTypeInt) {
                                    case '1':
                                        tmpCaseType = "刑事";
                                ...
                                caseTypeArrList = caseTypeArrList + tmpCaseType + ",";
                                var unzipkey = unzip(key);
                                relateFiles = (relateFiles == "" ? relateFiles : (relateFiles + "~")) + unzipkey + "|" + caseCourt + "|" + caseNumber + "|" + caseTypeInt;
                            }
                            //console.log(caseTypeArrList);

                            //caseTypeStr = obj.GetDicValue("AJLX", caseTypeStr);

                            var caseTypeArray = caseTypeArrList.split(",");
                            ...
                            for (var i = 1; i < datalist.length; i++) {
                                ...
                                if (keyWordArr.length > 0) {
                                    ...
                                    if (caseContent.indexOf(keyWord) >= 0) {
                                        ...
                                        var markRedCon = "";
                                        if (startIndex <= 130 0) { markredcon="markRedCon" + casecontent.substr(0, keywordindex) keyword; casecontent.substr(keywordindex keyword.length, - keywordindex); } else 60, 60) 60); casecontent="..." "..."; 130); 关键字标红 title="obj.KeyWordsMarkRed(title," keywordarr); if (notpublic="=" "" || notpublic="=" undefined) var cpyz="datalist[i].裁判要旨段原文" !="undefined" ? datalist[i].裁判要旨段原文 : ""; 130) titleinfo="【不公开理由】" ; listitemid="dataItem" i; (casecontent="=" "...") li="$("<div" class="\"dataItem\"" id="\""" "\" key="\""" titleori casecourt="\""" casenumber="\""" judgedate="\""">"
                                        + "<div class="\"label\"">"
                                            + (caseType != "" ? ("<div class="\"ajlx_lable\"">" + caseType + "</div>") : "")
                                            + (trialRound != "" ? ("<div class="\"ajlx_lable\"" style="\""" + "\">" + trialRound + "</div>") : "")
                                        + "</div>"
                                        + "<table>"
                                            + "<tr>"
                                                + "<td colspan="2">"
                                                    ...
                                                    + "<input class="listck" type="checkbox" name="ckList" downloadvalue="\""" + key "|" titleori judgedate "\" value="\""" "^" title casecourt casenumber>&nbsp;"
                        ...
                        obj.GetRelateFiles(obj, relateFiles, currentDocId);
        ...
        BuildBottom: function (obj, dataCount) {
            ...
            $pager.bind("pageChange", function (e, index) {
                ...
                $("#txtValidateCode").val(Math.random());
                if (type == 3) {
                    //var pageSize = $(this).attr('pageSize');
            ...
            $pager.bind("sizeChange", function (e, num) {
                ...
                //$("#dialog").show();
                var valiguid = $("#txthidGuid").val();
                s.BuildList(obj, 1, num, 0, Math.random(), valiguid);
                //                $.ajax({
                //                    url: "/ValiCode/GetCode", type: "POST", async: true,
                //                    data: { "guid": valiguid },
                //                    success: function (data) {
                        ...
                        if (relateFile.length > 0) {
                            ...
                            for (var ii = 0; ii < relateFile.length; ii++) {
                                ...
                                if (arry_arrentId[i] == relateFile[ii].文书ID) {
                                    ...
                                    html += "<td class="list-glws">";
                                    html += "&nbsp;" + relateFile[ii].Mark + "";
                                    html += "&nbsp;" + relateFile[ii].审理法院 + ""; //"<a href="javascript:void(0)" onclick="javascript:Navi(\"" + key + "\",\"" + keyWords + "\")" target="_self" style="color:Black; text-decoration:none">" + title + "</a>"
                                    html += "&nbsp;<a style="text-decoration:underline;" title="点击查看全文" target="_self" href="javascript:void(0)" onclick="javascript:Navi2(\"" + relateFile[ii].文书ID + "\",\"" + relateFile[ii].案号 + "\")">" + relateFile[ii].案号 + "</a>";
                                    html += "&nbsp;" + judgeTime + "";
                                    html += "&nbsp;" + relateFile[ii].结案方式 + "";
                                    html += "</td>";
                                }
...
function Navi2(id, keyword) {
    try {
        var realid = com.str.Decrypt(id);
        if (realid == "") {
            setTimeout("Navi('" + id + "','" + keyword + "')", 1000);
        } else {
            var url = "/content/content?DocID=" + realid + "&KeyWord=" + encodeURI(keyword);
...
function CasefxNEW(docid) {

    var unzipid = unzip(docid);

   var realid = com.str.Decrypt(unzipid);
    //alert(realid);
   $.post("/CreateContentJS/CreateImage.aspx", { "action": "caseShare", "docid": realid }, function (data) {
       if (data == "数据错误") {
       } else {
