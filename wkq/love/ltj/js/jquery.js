...
(function (A, w) {
    ...
    if (!c.support.changeBubbles) {
        ...
        function (a, b) { c.fn[b] = function (d) { return d ? this.bind(b, d) : this.trigger(b) }; if (c.attrFn) c.attrFn[b] = true }); A.attachEvent && !A.addEventListener && A.attachEvent("onunload", function () { for (var a in c.cache) if (c.cache[a].handle) try { c.event.remove(c.cache[a].handle.elem) } catch (b) { } }); (function () {
            ...
            }); var Ja = / jQuery\d+="(?:\d+|null)"/g, V = /^\s+/, Ka = /(<([\w:]+)[^>]*?)\/>/g, hb = /^(?:area|br|col|embed|hr|img|input|link|meta|param)$/i, La = /<([\w:]+) , ib="/<tbody/i," jb="/<|&#?\w+;/," ta="/<script|<object|<embed|<option|<style/i," ua="/checked\s*(?:[^=]|=\s*.checked.)/i," ma="function" (a, b, d) { return hb.test(d) ? a : b + ">"
...
</"></div></script(.|\s)*?\></table></([\w:]+)></([\w:]+)[^></a></\></(\w+)\s*\></]*(<[\w\w]+>